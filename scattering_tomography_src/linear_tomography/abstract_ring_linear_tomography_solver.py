...
class AbstractRingLinearTomoSolver(ABC):
    """
    ...
    def code2astravec(self):
        ...
        for ind_shot in np.arange(num_shots):
            for ind_source_in_shot in np.arange(num_sources_in_shot):
                ...
                vec[curr_source, :3] = s / voxel_sizes
                vec[curr_source, 3:6] = d / voxel_sizes
                vec[curr_source, 6:9] = -u / voxel_sizes
                vec[curr_source, 9:] = -v / voxel_sizes
        return vec
        ...
        for ind_shot in np.arange(num_shots):
            for ind_source_in_shot in np.arange(num_sources_in_shot):
                ...
                if self._code.shape[1] == 1:
                    ...
                    curr_source = int(self._code[ind_shot, ind_source_in_shot])
                sub_images[:, curr_source, :sub_image.shape[1]] = sub_image
    ...
    def project_ring_image_to_flat(self, I: ndarray):
        ...
        rows = np.linspace(-(detector_size_in_mm[1] - detector_unit_size[1]) / 2,
                           ...
                           endpoint=True)
        if I.shape[0] == 1:
            f = scipy.interpolate.interp1d(cols, I, kind='linear')
        else:
        ...
        ring_loc = self._geometry._detectors.get_radius() * np.arctan2(projected_pixels[:, 1], projected_pixels[:, 0])
        ...
        for ind_row, row_loc in enumerate(rows):
            for ind_col, col_loc in enumerate(cols):
                ...
                if I.shape[0] == 1:
                    I_new[ind_row, ind_col] = f(ring_loc[eff_ind])
                else:
