...
# projection code
full_code = np.loadtxt(os.path.join(get_experiment_path(cfg,GT_data=True), 'full_opt_code.txt'))
cfg['NUM_OF_SHOTS_GT'], cfg['NUM_PROJECTIONS_IN_SHOT'] = full_code.shape
...
code = NonTrivialProjectionCode(num_sources, num_shots, num_projection_in_shot)
code.load_full_multiplex_code(full_code, cfg)
...
vol_optimization = ADAM(oracle, x0, nIter=cfg["OPTIMIZATION_ITER"])
